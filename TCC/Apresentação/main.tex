...
\only<2>{
	\begin{itemize}
		\item Tomaremos $\matriz{M}$ ortogonal, unitária ou simplética tal que $\matriz{M} \in \Glin_N(\Se)$;
		
		\item $\exists$ $\matriz{D}$ diagonal e $\matriz{O} \in V_N(\Se^N)$ tais que $\matriz{M} = \matriz{O} \matriz{D} \matriz{O}^{-1}$.
	\end{itemize}
	\begin{block}{Medida Geral}
		Se $\matriz{D} = \diag(\mmany{\lambda}{N})$, a jpdf de $\matriz{M}$ pode ser expressa
		\[
...
\begin{frame}
	...
	\only<1,2>{
		...
		\onslide<2>{
		Existe $\mu_{V}^* = \arg \inf {\mathcal{H}_N(\mu_{V})}$ medida de equilíbrio única no limite termodinâmico $N \rightarrow \infty$ que satisfaz
		\[
