...
\begin{frame}[t]
	...
	\only<1-5>{
		...
		\onslide<3-4>{
			...
			\only<4>{
				Assintoticamente em $N$, $S^{\mu_V}(z) = \int \frac{\mu^*_V(\lambda)}{z - \lambda} \dd \lambda$ e $\Pi(z) = \int \frac{\V'(z) - \V'(\lambda)}{z - \lambda} \mu^*_V(\lambda) d\lambda$
...
%	}
	...
	\only<1-2>{
		...
		\onslide<1>{
			$$\p(q,p) = \p_q \otimes \p_p = \ee^{-\beta_N \Ee_N(q,p)}/Z'_N$$
			é medida de probabilidade inversível e invariante do processo de difusão de Markov solução da dinâmica: 
		}
		\onslide<2>{
...
\begin{frame}
	...
	\begin{figure}
		...
		\caption{Medidas de equilíbrio simuladas para o potencial proposto}
	\end{figure}
	}
\end{frame}
%---------------------------------------------------------
...
\begin{frame}
	...
	\begin{figure}
		...
		\caption{Diagrama dos suportes obtidos pela simulação do potencial}
	\end{figure}
	}
	...
	\begin{figure}
		\centering
		\includegraphics[width=\textwidth]{./media/Results/allshapesalt}	
		\caption{Diagrama dos suportes obtidos pela simulação do potencial - Sem escala}
	\end{figure}
	}
	...
	\begin{figure}
		...
		\caption{Diagrama de fase do Artigo de \cite{bleher2016mother}}
	\end{figure}
	}
	...
	\begin{figure}
		\centering
		\includegraphics[width=0.6\textwidth]{./media/Results/mashedshapesalt}	
		\caption{Validação dos resultados com diagrama de fase de \cite{bleher2016mother}}
	\end{figure}
	}
\end{frame}
