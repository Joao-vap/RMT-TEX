...
\end{equation}
where $\dd A(z) \deff \dd^2 z / \pi$ is the area measure. For this measure, the \textit{Partition Function} is given by
\begin{equation}
	\pZ{N}{(\beta)} \deff \int_{\C^N} \prod_{j > k = 1}^{N} |z_j - z_k|^\beta \prod_{j=1}^{N} \ee^{- \frac{\beta N}{2} Q(z_j)} \dd A(z_j).
\end{equation}

\begin{definicao}
	Let $\p_N^{(\beta)}$ be as in \eqref{Equation: Measure Normal Matrix}. If $\beta = 2$, $\p_N^{(2)}$ is the measure for the ensemble we call \textbf{random normal matrix}. If, for example, one sets $Q(z) = |z|^2$ this is called \textbf{complex Ginibre ensemble}.
\end{definicao}

We also define the measure for the configurational canonical Coulomb gas ensemble in the upper-half plane  - which has an additional complex conjugation symmetry and is governed by the law 
\begin{equation}
...
\begin{definicao}
	Let $\p_N^{(\beta)}$ be as in \eqref{Equation: Measure Symplectic Matrix}. If $\beta = 2$, $\p_N^{(2)}$ is the measure for the ensemble we call  \textbf{planar symplectic ensemble}. If, for example, one sets $Q(z) = |z|^2$ this is called \textbf{symplectic Ginibre ensemble}.
\end{definicao}
...
which satisfies some nice growth and smoothness conditions, as specified in \autoref{Section: Working Model}. We begin by quickly considering the auxiliary result.
...
\end{equation}
are the norming constant for the monomial family of orthogonal polynomials in respect to $\wf$ as in \eqref{Equation: Weight Function}. We also write, for the planar symplectic ensemble 
\begin{equation}
...
With this two radius $r_0, r_1$ we defined the droplet as the region composed by every point $z$ such that $r_0 \leq |z| \leq r_1$. Because of the radial symmetry, one can take some function $q: \R \rightarrow \R$ such that $q(|z|) = Q(z)$; If we say that the $Q$ is sub-harmonic on the plane, i.e., $\Delta Q(z) \geq 0$ for all $z \in \C$, then
\begin{equation}
	\Delta Q(z) \geq 0 = \frac{1}{r} (r q'(r))' \geq 0 \implies rq'(r) \ \text{is increasing} \ \forall r \in \R,
\end{equation}
and strictly increasing in the droplet by the same reason. 
