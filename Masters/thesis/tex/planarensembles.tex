We are going to discuss in this chapter the ideas developed in the paper \citeonline{Byun_2023}, where asymptotics were developed for a large class of random matrix ensembles using relatively straightforward asymptotic techniques; taking advantage of the orthogonal polynomials present in these models. In the end, we hope to clarify the origin for the novel limit-behavior expression for the \textit{Partition Function} $\pZ{N}{\beta=2}$ in the shape of the expression
\begin{equation}
	\ln(\pZ{N, \beta=2}{Q}) = -I_Q N^2+\frac{1}{2} N\ln N+E_Q N+G\ln N+F^Q + o(1).
\end{equation}

Partition functions perform the role of the normalization for the probability measure relative to the Coulomb gas being worked on - a system of interacting particles confined by some potential as described in \autoref{Section: Coulomb Gas}. In this instance, we deal with complex plane gases. This relates in RMT, to the \textit{random normal matrix} and \textit{planar symplectic ensembles}, depending on the symmetry imposed. If one takes, for example, the quadratic potential $Q(x) = x^2/2$ using the $2-\beta$ regime, one gets the \textit{complex} and \textit{planar} Ginibre ensembles, falling back into the classical models. For the results presented, another restriction is made; the confining potentials must be radially symmetric. For a large class of such potentials $Q$, the first term $I_Q$ in the expression was known for a long time, given by the system's energy when distributed as the associated equilibrium measure. The coefficient $1/2$ of the order $N\ln N$, as well as the entropy term $E_Q$, were described in a recent work of \citeonline{Leble2017-yj}, also for rather large classes of $Q$. The remaining terms in this expansion are new. The term $G$ appears to depend only on whether the limiting spectrum is an annulus or a disk which, surprisingly, seems to not have been observed in the previous literature. 
...
In this section we properly define the model and specifics of the working model to which we wish to get the asymptotics from. We have discussed in \autoref{Section: Coulomb Gas} how eigenvalue statistics can be interpret as some Coulomb gas. On the complex plane one could have for example the measure
\begin{equation}
...
\begin{definicao}
	Let $\p_N^{(\beta)}$ be as in Equation \ref{Equation: Measure Normal Matrix}. If $\beta = 2$, $\p_N^{(2)}$ is the measure for the ensemble we call \textbf{random normal matrix}. If, for example, one sets $Q(z) = |z|^2$ this would be called \textbf{complex Ginibre ensemble}.
\end{definicao}
...
\begin{definicao}
	Let $\p_N^{(\beta)}$ be as in Equation \ref{Equation: Measure Symplectic Matrix}. If $\beta = 2$, $\p_N^{(2)}$ is the measure for the ensemble we call  \textbf{planar symplectic ensemble}. If, for example, one sets $Q(z) = |z|^2$ this would be called \textbf{symplectic Ginibre ensemble}.
\end{definicao}
...
which satisfies some nice growth and smoothness conditions, as specified in \autoref{Section: Working Model}. Because of the radial symmetry, one can take some function $q: \R \rightarrow \R$ such that $q(|z|) = Q(z)$; If we say that the $Q$ is sub-harmonic on the plane, i.e., $\Delta Q(z) \geq 0$ for all $z \in \C$, then
\begin{equation}
...
\end{lema}
\begin{proof}
	We start stating the following equality which follows from Theorem \ref{Theorem: Kernel} that
	\begin{align}
		\det\left[ K_N^{(2)}(\lambda_i, \lambda_j) \right]_{i,j=1}^{N} & =  p^{(w)}(\mmany{\lambda}{N};\beta), \\
		\prod_{k=1}^N h_k^{-1}  \det\left[ \sqrt{\wf(x) \wf(y)} \sum_{k=0}^{N-1} q_k^{(2)}(x) q_k^{(2)}(y) \right]_{i,j=1}^{N} & = \prod_{1 < i \leq j < N} (\lambda_j - \lambda_i)^2 \prod_{k=0}^{N} w(\lambda_k)
	\end{align}
	therefore
	\begin{equation}
		N! \det\left[ \tilde{K}_N^{(2)}(\lambda_i, \lambda_j) \right]_{i,j=1}^{N} = N! \prod_{k=1}^N h_k \prod_{1 < i \leq j < N} (\lambda_j - \lambda_i)^2 \prod_{k=0}^{N} w(\lambda_k)
	\end{equation}
	where
	\begin{equation}
		\pZ{N,2}{} = N! \prod_{k=1}^N h_k.
	\end{equation}
\end{proof}
...
\end{equation}
are the norming constant for the monomial family of orthogonal polynomials in respect to $\wf$ as in Equation \ref{Equation: Weight Function}. One could also write, for the planar symplectic ensemble 
\begin{equation}
...
\begin{lema}
	...
	\end{equation}
	then, with $(1)$ as in Equation \ref{Equation: Separation annulus} and $0 \leq j \leq N-1$, asymptotically on $N$,
	\begin{equation}
...
\begin{lema}
		For the second part of the integral we have, for $(2)$ as in Equation \ref{Equation: Separation annulus} and $0 \leq j \leq N-1$, asymptotically on $N$,
		\begin{equation}
...
\begin{proof}
	...
	Notice that the error term of Lemma \ref{Lema: error annulus} is absorbed into the error order of Equation \ref{Equation: main part annulus}. Therefore we have the asymptotic expression for $h_j$ given by Equation \ref{Equation: Asymptotic hj annulus} and the proof for Theorem \ref{Theorem: hj annulus} follows directly.
\end{proof}

%\subsubsection{Asymptotics on the Summation}

We return to Equation \ref{Equation: Partition Function} and turn to the calculation of each term of $	\sum_{j=0}^{N-1} \ln{h_j}$ using the logarithm of the expansion calculated previously in Theorem \ref{Theorem: hj annulus}
\begin{equation}
...
The proof comes from gathering the terms in Equation \ref{Equation: sum log annulus} with its respective expressions derived in Lemmas \ref{Lemma: Annulus sum N2} and \ref{Lemma: Annulus Sum 1/2}. We go term by term. Lemma \ref{Lemma: Annulus sum N2} refers to the asymptotic of 
\begin{equation}
...
\begin{proof}
	...
	\end{multline}
	The first term in Equation \ref{Equation: EulerMaclaurin 1 annulus} is solved doing a integral by parts. We remember the expression for the derivative of the potential $V'_\tau(r) = q'(r) - 2\tau/r$. Note that in the critical point $r_{\tau(j)}$, we should have $r_{\tau(j)} q'(r_{\tau(j)}) = 2 \tau(j)$. This implies that, for $r(\tau) \deff r_\tau$, the differential can be written as
	\begin{align}
	...
	\end{equation}
	The proof for the summation $\sum_{j=0}^{N-1} V_{\tau(j)}(r_{\tau(j)})$ is complete if one gathers the terms in Equation \ref{Equation: Equilibrium energy annulus}, Equation \ref{Equation: log annulus} and Equation \ref{Equation: U function annulus},
...
\begin{proof} (Theorem \ref{Theorem: Annulus Free Energy})
	...
	\begin{align}
		...
		\end{multlined} \\
		& 
		\deff - N^2 I_Q[\mu_Q] + \frac{N}{2} \left( \ln(2\pi) - E_Q[\mu_Q] - 2\right) + \frac{N}{2} \ln(N) + \frac{\ln(2\pi)}{2} + \frac{1}{2} \ln(N)+ F_V[A_{[r_0, r_1]}] + \Boh(N^{-1}),
	\end{align}
...
In this case, we consider $r_0 = 0$. As before, the measure in Equation \ref{Equation: Measure Normal Matrix} to be the law for distribution of the particles in this model. When we say $r_0 = 0$, we mean that the support for this configuration space of particles is a disk in the complex plane. The expression we get is different from taking the limit in the case $r_0 > 0$. \cite{Byun_2023}
...
Again, we start by studying the asymptotic for $h_j$ and then for the summation $\sum \ln(h_j)$. This theorem then should follow from the following lemmas concerning this two asymptotics. In the asymptotic expansions of $h_j$, one should distinguish the following two cases depending on a small constant $1/5 > \epsilon > 0$. 
...
\begin{proof}
	...
	\begin{equation}
		...
		\end{cases}
	\end{equation}
	since $q$ has critical point at $r_\tau = 0$. This means that the assumptions in the Implicit Function Theorem \ref{Theorem: Implicit Function} do not hold. We must use the \textit{blow-up} technique. Consider a auxiliary variable $v \deff v(r)$ such that $v(r) \tau = r^2 $ and look for solutions of the redefined
	\begin{equation}
...
\begin{lema}
	...
	\begin{equation}
		\int_{|r - r_j| > \delta_N} \left( \ee^{-q(r)} r^{2} \right)^M r \dd r < \infty,
	\end{equation}
	then, with $(2)$ as in Equation \ref{Equation: Separation disk}, for the error-bound integral we have the asymptotic for $N$ given by
	\begin{equation}
...
\begin{align}
	...
	\label{Equaition: expansion Vt1/2>}	
\end{align}
...
\end{equation}
one can rewrite using Equations \ref{Equaition: expansion Vt1/2<} and \ref{Equaition: expansion Vt1/2>},
\begin{equation}
...
\begin{lema}
	...
	\end{equation}
	where $(1)$ is as in Equation \ref{Equation: Separation disk}, $\mathfrak{B}(r_{\tau(j)})$ is as in Theorem \ref{Theorem: Disk Free Energy}	and
	\begin{equation}
		\epsilon_{N,1} \deff \Boh(j^{-\frac{3}{2}} (\ln N)^\alpha)
	\end{equation}
	for some $\alpha > 0$.
...
\begin{align}
	...
	\end{multlined}
\end{align}
where  $\epsilon_{N,1} = \Boh(j^{-\frac{3}{2}} (\ln N)^\alpha)$ for some $\alpha > 0$. From here, the expression can be directly identified with Equation \ref{Equation: value-contributing integral} and the techniques for getting the result are one-to-one with the ones already done in that case.
then, distributing the integral,
\begin{multline}
		(1) = 2 r_{\tau(j)} \frac{\ee^{-N V_{\tau(j)}(r_{\tau(j)})}}{\sqrt{N}}
\left[
\left(- \frac{V^{(4)}_{\tau(j)}(r_{\tau(j)})}{24 N} - \frac{V^{(3)}_{\tau(j)}(r_{\tau(j)})}{6 r_{\tau(j)} N} \right)
\int_{-\ln(N)}^{\ln(N)} \ee^{-\frac{y^2}{2} V^{(2)}_{\tau(j)}(r_{\tau(j)})} y^4 \dd y \right. \\
\left. - \frac{V^{(3)}_{\tau(j)}(r_{\tau(j)})^2}{72 N}
...
At this point, the result can be directly evaluated using the Gaussian integrals in Proposition \ref{Prop: Gaussian integrals}.	So that we can finally substitute the values to get
\begin{multline}
	(1) = \sqrt{2\pi} r_{\tau(j)} \frac{\ee^{-N V_{\tau(j)}(r_{\tau(j)})}}{\sqrt{N}} \left[
	\left(- \frac{V^{(4)}_{\tau(j)}(r_{\tau(j)})}{24 N} - \frac{V^{(3)}_{\tau(j)}(r_{\tau(j)})}{6 r_{\tau(j)} N} \right) \frac{3}{16} \left( \frac{4}{V^{(2)}_{\tau(j)}(r_{\tau(j)})} \right)^{\frac{5}{2}} \right. \\
	\left. - \frac{V^{(3)}_{\tau(j)}(r_{\tau(j)})^2}{72 N} \frac{15}{64} \left(\frac{4}{V^{(2)}_{\tau(j)}(r_{\tau(j)})} \right)^{\frac{7}{2}} +
...
\end{equation}
Due to strict sub-harmonicity of Q in a neighborhood of the droplet, $r_\tau$ satisfies $r_\tau = \Boh (\tau^{\frac{1}{2}})$ as $\tau \rightarrow 0$, this is given by Proposition \ref{Proposition: Behavior rt}. Since $V_\tau$ has a global minimum at $r = r_\tau$ and increases for larger values, for all $r > r^*_{\tau(j + 1)}$ we have
\begin{align}
...
So now we need to return to  Equation \ref{Equation: Partition Function} and calculate each term for $	\sum_{j=0}^{N-1} \ln{h_j}$ using the expansion in terms of Theorem \ref{Theorem: hj disk j>mn} and \ref{Theorem: hj disk j<mn} for the expression
...
\begin{proposicao}
	...
	\end{multline}
	Here, $I_Q[\mu_Q] $ is as in Theorem \ref{Theorem: Disk Free Energy}, $\epsilon$ is as in the definition for $m_N$, and
	\begin{equation}
...
\end{align}
Therefore, for this first term, gathering the above and expanding using Proposition \ref{Proposition: Behavior rt} and Equation \ref{Equation: behavior q(rt)},
\begin{align}
	& \begin{multlined}
...
\begin{proposicao}
	...
	\end{equation}
	where $E_Q[\mu_Q]$ is as in Theorem \ref{Theorem: Disk Free Energy} and $\epsilon$ is as in the definition for $m_N$.
...
\begin{proof}
	...
	\end{align}
	Ultimately, using Equation \ref{Equation: behavior ln rt mn} and Equation \ref{Equation: behavior q(rt)}, 
	\begin{multline}
...
\begin{proof} (Lemma \ref{Lemma: Summation disk >mn})
	We start by stating from Equation \ref{Equation: Summation on B}, with the new limits,
	\begin{multline}
	...
	\end{multline}
	Using Proposition \ref{Proposition: Behavior rt} we write that
	\begin{equation}
