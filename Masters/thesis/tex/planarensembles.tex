...
\begin{definicao}
	Let $\p_N^{(\beta)}$ be as in \eqref{Equation: Measure Normal Matrix}. If $\beta = 2$, $\p_N^{(2)}$ is the measure for the ensemble we call \textbf{random normal matrix}. If, for example, one sets $Q(z) = |z|^2$ this would be called \textbf{complex Ginibre ensemble}.
\end{definicao}
...
\end{equation}
are the norming constant for the monomial family of orthogonal polynomials in respect to $\wf$ as in \eqref{Equation: Weight Function}. One could also write, for the planar symplectic ensemble 
\begin{equation}
...
\begin{lema}	\label{Lema: error annulus}
	...
	\end{equation}
	then, with $(1)$ as in \eqref{Equation: Separation annulus} and $0 \leq j \leq N-1$, asymptotically on $N$,
	\begin{equation}
...
We now move on to the value-contributing part of the integral. The other order term should be absolved in this one if \autoref{Theorem: hj annulus} is to be right.
	
\begin{lema} 	\label{Lema: value annulus}
		For the second part of the integral we have, for $(2)$ as in \eqref{Equation: Separation annulus} and $0 \leq j \leq N-1$, asymptotically on $N$,
		\begin{equation}
...
\begin{proof}
	...
	\end{multline}
	At this point, the result can be directly evaluated using the Gaussian integrals in \autoref{Prop: Gaussian integrals}.	So that we can finally substitute the values to get
	\begin{multline}
...
\begin{proof}
	...
	\end{multline}
	The first term in \eqref{Equation: EulerMaclaurin 1 annulus} is solved doing a integral by parts. We remember the expression for the derivative of the potential $V'_\tau(r) = q'(r) - 2\tau/r$. Note that in the critical point $r_{\tau(j)}$, we should have $r_{\tau(j)} q'(r_{\tau(j)}) = 2 \tau(j)$. This implies that, for $r(\tau) \deff r_\tau$, the differential can be written as
	\begin{align}
...
We can finally prove \autoref{Theorem: Annulus Free Energy}.
...
In this case, we consider $r_0 = 0$. As before, the measure in \eqref{Equation: Measure Normal Matrix} to be the law for distribution of the particles in this model. When we say $r_0 = 0$, we mean that the support for this configuration space of particles is a disk in the complex plane. The expression we get is different from taking the limit in the case $r_0 > 0$. \cite{Byun_2023}
...
\begin{lema}
	...
	\begin{equation}
		\int_{|r - r_j| > \delta_N} \left( \ee^{-q(r)} r^{2} \right)^M r \dd r < \infty,
	\end{equation}
	then, with $(2)$ as in \eqref{Equation: Separation disk}, for the error-bound integral we have the asymptotic for $N$ given by
	\begin{equation}
...
\end{equation}
one can rewrite using \eqref{Equation: expansion Vt1/2<} and \eqref{Equation: expansion Vt1/2>},
\begin{equation}
...
\begin{lema}
	...
	\end{equation}
	where $(1)$ is as in \eqref{Equation: Separation disk}, $\mathfrak{B}(r_{\tau(j)})$ is as in \autoref{Theorem: Disk Free Energy}	and
	\begin{equation}
		\epsilon_{N,1} \deff \Boh(j^{-\frac{3}{2}} (\ln N)^\alpha)
	\end{equation}
	for some $\alpha > 0$.
...
\begin{align}
	...
	\end{multlined}
\end{align}
where  $\epsilon_{N,1} = \Boh(j^{-\frac{3}{2}} (\ln N)^\alpha)$ for some $\alpha > 0$. From here, the expression can be directly identified with \eqref{Equation: value-contributing integral} and the techniques for getting the result are one-to-one with the ones already done in that case.
then, distributing the integral,
\begin{multline}
...
\end{align}
where we can expand some terms using the following results given by \autoref{Proposition: Behavior rt}
\begin{equation}
...
We can finally prove \autoref{Theorem: Disk Free Energy}.
