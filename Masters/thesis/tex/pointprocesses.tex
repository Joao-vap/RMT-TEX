...
\begin{exemplo}
	...
	\end{equation}
	as given by Equation \ref{Equation: Symmetric correlation}.
\end{exemplo}
...
\end{equation}
as long as the right and side is well defined. Important examples of DPPs are constructed by using the following result.

\begin{teorema} (Construction for DPP, \cite{ManuelaGirottiPhD})
	...
	\end{equation}
	is a probability measure on $\R^n$, invariant under coordinate permutations. The associated point process is a determinantal point process with $K$ as a correlation kernel.
\end{teorema}
...
\begin{demonstracao}
	...
	\end{equation}
	We see from Equation \ref{Equation: premisse last particle} and the dominated convergence theorem that we can let $s \rightarrow \infty$ and obtain Equation \ref{Equation: conclusion last particle}.
\end{demonstracao} 
}
...
\begin{proposicao} \cite{ManuelaGirottiPhD}
	Let K be the correlation kernel of a DPP and let $c$ be a point such that $0 < K(c, c) < + \infty$. Then, the point process obtained by removing $c$ is determinantal with kernel
	\begin{equation}
