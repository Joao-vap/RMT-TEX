...
The definitions given in this section and some ideas follow very closely the well structured works in \cite{SERFOZO19901}, \cite{JOHANSSON20061} and \cite{ManuelaGirottiPhD}.
...
\begin{exemplo}
		...
		Assume that $b$, $e$ and $c$ are bulk, edge and cusp points of some density $\rho$ with parameters $\gamma_e$, $\gamma_c$ defined in such a way that
		\begin{equation}
		...
		\begin{align}
			\frac{1}{\rho(b)^k} \p_k^{n}\left(b + \frac{x}{\rho(b) n }\right) & \approx \det{\left(\frac{\sin{\pi(x_i - x_j)}}{\pi(x_i - x_j)} \right)_{i,j \in [k]}}, \quad (Bulk) \\
			\frac{n^{k/3}}{\gamma_e^k} \p_k^{n}\left(e + \frac{x}{\gamma_e n^{2/3} }\right) & \approx \det{\left(K_{\text{Airy}}(x_i, x_j) \right)_{i,j \in [k]}}, \quad (Edge) \\
			\frac{n^{k/4}}{\gamma_c^k} \p_k^{n}\left(c + \frac{x}{\gamma_c n^{3/4} }\right) & \approx \det{\left(K_{\text{Pearcey}}(x_i, x_j) \right)_{i,j \in [k]}}. \quad (Cusp)
		\end{align}
\end{exemplo}