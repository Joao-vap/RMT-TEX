...
\begin{equation}
	B_n(x + h) = \sum_{k=0}^{n} {n \choose k} B_{k}(x) h^{n-k},
\end{equation}
that give us that $B_0(x) = 1$. More interestingly for us is the fact that these polynomials satisfy the differential equation 
\begin{equation}	\label{Equation: Differential Bernoulli}
	B'_n(x) = n B_{n-1}(x), \quad n = 1,2,\cdots
\end{equation}
Finally, the periodic Bernoulli polynomial is simply the Bernoulli polynomials evaluated at the fractional part of the argument, i.e., $\tilde{B}_n(x) = B_n(x - \floor{x})$. Now, let $B_n$ and $B_n(x)$ denote the $n$-th Bernoulli number and polynomial, respectively, and $\tilde{B}_n(x)$ the $n$-th Bernoulli periodic function $B_n(x - \floor{x})$. Assume that $a, b$ are reals and $p$ is an integer such that $b > a$, $p > 0$, and $f^{(2k)}(x)$ is absolutely integrable over $[a,b]$. Then, 

\begin{teorema} \label{Theorem: Euler-Maclaurin}(Euler-Maclaurin Formula,  \cite{Trapezoidal-Maclaurin})
	For some function $f \in C^p(\R)$ and $p \geq 1$ let $h = (b - a)/n$. Then, the Euler-Maclaurin formula for the interval $[a,b]$ is given by
	\begin{multline}
		h \sum_{j=1}^{n-1} f(a + j h) = \int_{a}^{b} f(x) \dd x - \frac{h}{2} (f(b) - f(a)) + \sum_{r = 1}^{p-1} \frac{h^{r+1} B_{r+1}}{(r+1)!} \left[ f^{(r)}(b) - f^{(r)}(a) \right] \\- h^{p+1} \int_0^1 \frac{B_{p}(1-x)}{(p)!} f^{(p)}(c+xh) \dd x
	\end{multline}
%	\begin{multline}
%		\sum_{j=m}^{n} f(j) = \int_{m}^{n} f(x) \dd x + \frac{1}{2} \left(f(m) + f(n)\right) + \sum_{s=1}^{k-1} \frac{B_{2s}}{(2s)!} \left( f^{(2s-1)}(n) - f^{(2s-1)}(m)\right) \\ + \int_{m}^{n} \frac{B_{2k} - \tilde{B}_{2k}(x)}{(2k)!} f^{(2k)}(x) \dd x.
%	\end{multline}
\end{teorema}
...
\begin{align}
	\int_{a}^{b} f(x) \dd x & \approx h \sum_{j = m}^{n-1} f(j^*) = h \sum_{j = m}^{n-1} \frac{f(j+1) + f(j)}{2}, \\
	& = h \sum_{j = m+1}^{n-1} f(j) + \frac{h}{2} \left( f(n) + f(m) \right) = h \sum_{j = m}^{n-1} f(j) + \frac{h}{2} \left( f(n) - f(m) \right) .
\end{align}
...
\begin{align}
	...
	& 
	\begin{multlined}
		= h \frac{f(c) + f(c+h)}{2} - \sum_{r = 1}^{p-1} \frac{h^{r+1} B_{r+1}}{(r+1)!} \left[ f^{(r)}(c+h) - f^{(r)}(c) \right] \\
...
\begin{align}
	h & \left[ \frac{f(a)}{2} + f(a+h) + \cdots + f(b-h) + \frac{f(b)}{2} \right] = \frac{h}{2} (f(b) - f(a)) + h \sum_{j=1}^{n-1} f(a + j h), \\
	& = \int_{a}^{b} f(x) \dd x + \sum_{r = 1}^{p-1} \frac{h^{r+1} B_{r+1}}{(r+1)!} \left[ f^{(r)}(b) - f^{(r)}(a) \right] - h^{p+1} \int_0^1 \frac{B_{p}(1-x)}{(p)!} f^{(p)}(c+xh) \dd x. 
\end{align}
\end{proof}
...
\begin{proof}
	Although the proof may be similar there is an important distinction in the solution. Suppose we wanted to try the same approach. We first assume, w.l.g., that c = 0. Then, we write
	\begin{align}
	...
	\end{equation} 
	However we can not apply \autoref{Theorem: Implicit Function} as the partial derivative $\partial_t H = \phi'$ vanishes at $t=0$. A nice fact about this indetermination is that it does not come from the fact that there are no solutions to $H(t,s) = 0$ but from the fact that there are two. To solve this we will use a technique called \textit{blow up}. First, we introduce a variable $v \deff v(t)$ such that $vs = t$. Now, we redefine our function of interest to be
	\begin{equation}
		L(s, v) \deff \frac{\phi(sv) - \phi(0)}{s^2} - 1.		
	\end{equation}
	for $s\in(-\delta, \delta) - \{0\}$. We think this function is also singular but note that, using the Taylor Expansion of $\phi$ we write
	\begin{equation}
...
\end{equation}
and when $\lambda$ grows large, we have the exponentially growing/decaying term $\ee^{-\lambda u(z)}$ and a oscillating term coming from $\ee^{\ii \lambda v(z)}$ which is hard to control. The idea to get around this fact is to use paths in which this term is constant. If we have such a case, we write
\begin{multline}
...
\begin{proof}
	...
	\begin{equation}
		\int_{a}^{b} (p(\gamma(t))\alpha'(t)-q(\gamma(t))\beta'(t)) \ee^{-\lambda u(\gamma(t))} \dd t = \sqrt{2\pi} \ee^{-\lambda u(z_0)} \left( \frac{p(z_0)\alpha'(c)-q(z_0)\beta'(c)}{\sqrt{|(u \circ \gamma)''(c)|} \lambda^{1/2}} + \epsilon \right)
	\end{equation}
	and
	\begin{equation}
		\int_{a}^{b} (p(\gamma(t))\beta'(t)-q(\gamma(t))\alpha'(t)) \ee^{-\lambda u(\gamma(t))} \dd t = \sqrt{2\pi} \ee^{-\lambda u(z_0)} \left( \frac{p(z_0)\beta'(c)-q(z_0)\alpha'(c)}{\sqrt{|(u \circ \gamma)''(c)|} \lambda^{1/2}} + \epsilon \right)
	\end{equation}
	with $ \epsilon = \Boh \left(\lambda^{-3/2}\right)$. Using that $v_0 + u(z_0) = f(z_0)$ and also that,
	\begin{equation}
...
\begin{exemplo}
	...
	\begin{equation}
		\begin{split}
			\He_n(N^{\frac{1}{2}} \alpha) & = \frac{(N-1)!}{2\pi \ii} \int_{\Gamma} \ee^{(N^{1/2} \alpha)(N^{1/2} \beta)- \frac{1}{2}(N^{1/2} \beta)^2 - N\ln{(N^{1/2} \beta)}} \dd \beta \\
			& =  \frac{(N-1)! N^{\frac{1}{2}\left(1-N\right)}}{2\pi \ii} \int_{\Gamma}  \ee^{-N(- \alpha \beta+ \frac{1}{2} \beta^2 + \ln{(\beta)})} \dd \beta,
		\end{split}
	\end{equation}
	so that we have
	\begin{equation}
		\He_n(\sqrt{N}\alpha) = C_n^0 \int_{\Gamma}  \ee^{-N \phi_\alpha(\beta) } \dd \beta, \quad
	...
	each of the cases refers to one of the regimes we described before.
		...
		\centering
		\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1cm,y=1cm, rotate=-90, auto]
			\begin{axis}[
				%title={$\alpha > 2$},
				xmin=-2, xmax=2,
				ymin=-2, ymax=2,
				...
				axis equal]					
				\node [rotate=90] at (axis cs:0.7,0.7) {$t_{c_+}$};
				\node [rotate=90] at (axis cs:-0.7,0.7) {$t_{c_-}$};
		...
		\end{subfigure} \hfill
				\begin{subfigure}{0.3\textwidth}
			\centering
			\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1cm,y=1cm, rotate=-90, auto, transform shape ]
				\begin{axis}[
					%title={$\alpha = 2$},
					xmin=-2, xmax=2,
					ymin=-2, ymax=2,
					...
					\node [rotate=90] at (axis cs:0,1) {$t_{c_{\pm}}$};
		...
		\begin{subfigure}{0.3\textwidth}
			...
			\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1cm,y=1cm, rotate=-90, auto, transform shape ]
				\begin{axis}[
					...
					\node [rotate=90] at (axis cs:0,1.1) {$t_{c_+}$};
					\node [rotate=90] at (axis cs:0,1/2+0.1) {$t_{c_-}$};
