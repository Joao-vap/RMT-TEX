...
We have seen that the study of eigenvalues of random matrices is of interest and can be seen as a point process. For some specific models, one can further find a more rigid structure related to determinantal point processes. These allow us to see eigenvalue distributions in terms of some correlation Kernel much easier to deal with than the initial formulation. Now, we will see that eigenvalue dynamics for invariant models can be though of as another DPP: The so called \textit{Coulomb gas}. These can be though of as a physical gas of repelling ions restricted to some conductor under some confining potential. Although it may be daunting to have yet another model to work with, these gases lead us directly to some nice interpretations and results. Namely, one can think of \textit{thermodynamic equilibrium} for a large enough number of particles and this makes complete sense in a physical setting. In our matrices, these equilibrium distributions refer to the eigenvalue distribution, a very interesting characterization of a RMT model. On this chapter we will expand on these ideas.
...
\begin{definicao}
	...
	\end{equation}
	is usually called the \textbf{Hamiltonian}, or more concretely, the \textbf{configurational energy} of the system. The function $\g \colon \Sigma \rightarrow (-\infty, \infty]$ is the \textit{Coulomb Interacting Kernel}, solution to the Poisson equation given by 
	\begin{equation}
		g_{d-2}(\vec{x})
		\begin{cases}
		= - \ln(|\vec{x}|), \quad \text{if} \ d-2 = 0, \\
		= - \frac{1}{s|\vec{x}|^s}, \quad \text{if} \ d-2 \neq 0.
		\end{cases}
	\end{equation}
...
\begin{equation}
	\dd \p_n(\mmany{x}{n}) = \frac{e^{-\beta n^2 \Hf_n(\xi_n)}}{\pZ{n}{\beta}} \prod_{i=1}^n \dd x_i = \frac{1}{\pZ{n}{\beta}}\prod_{1\leq j < k \leq n} |x_j - x_k|^\beta \prod_{j=1}^{n} \ee^{- \beta n \Q(x_j)} \dd x_j.
\end{equation}
This is to say that eigenvalues from invariant ensembles behave as a Coulomb gas. This opens the way for interpretations on the distribution of eigenvalues but also to computations using simulation methods for particle dynamics.
...
Especially interesting is the case when $n$ tends to a large enough number. The description of the eigenvalues as a Coulomb gas and as a point process in such limit lead us to the idea of minimizing some type of energy of the system by finding equilibrium configurations  $\xi_n = \{\mmany{x}{n}\}$; one can think of the thermodynamic idea of equilibrium in gas systems.

To clarify the point, suppose we are dealing with the density
\begin{equation}
	\p_n(\xi_n) = \frac{e^{-\frac{\beta n^2}{2} \Hf_n(\chi_n)}}{Z_{n,\beta}}, \quad \Hf_n(\chi_n) = \frac{2}{n} \sum_{i = 1}^{n} Q(x_i) + \frac{1}{n^2} \sum_{i \neq j}^{n} \ln{\frac{1}{|x_i - x_j|}}. 
\end{equation}
...
\end{equation}
The quantity $E^{(Q)}$ is the \textit{equilibrium energy} of the system and $I_Q(\mu_x)$ is the \textit{energy integral}. However, to analyze this case, we first consider the simplified case without an external potential. 
...
\end{proposicao}
\begin{proof}
	Firstly, property $(1)$ follows directly by $(2)$, since a lower semi-continuous function is everywhere $> -\infty$ and $\mu$ has compact support. Property $(2)$ can be proven noticing that
	\begin{equation}
...
\begin{align}
	...
	&  = I^{\wf} (\mu^{\wf}),
\end{align}
one can define the so-called \textit{Euler-Lagrange} type variational equations. They are important as they provide a tool, for some cases, to solve these equations for the equilibrium measure $\mu^{\wf}$.
...
\begin{proof}
	...
	\begin{equation}
		l \leq \int \left[ 2 \int \ln{|x-y|^{-1}} \dd \mu^\wf(y) + 2 \Q(x) \right] \dd \mu^*_B(x) < l
	\end{equation}
...
The most important thing to notice here is that \eqref{Equation: logarithmic energyy - entropy} is of the form required for the Laplace asymptotic technique discussed in \autoref{chapter:asymptotics}. This means that it can be estimated, if $S$ has a maxima with strictly negative second derivative, by the evaluation at its maximum point. One can find 
\begin{align}
