...
Recall the expression for invariant ensembles in Definition c{Definition: polynomial ensemble}, note that, rearranging the terms in \eqref{Equation: Coulomb Gas Measure} and considering a logarithm interaction kernel we get
\begin{equation}
...
The most important thing to notice here is that \eqref{Equation: logarithmic energyy - entropy} is of the form required for the Laplace asymptotic technique discussed in \autoref{chapter:asymptotics}. This means that t can be estimated, if $S$ has a maxima with strictly negative second derivative, by the evaluation at its maximum point. One can find 
\begin{align}
