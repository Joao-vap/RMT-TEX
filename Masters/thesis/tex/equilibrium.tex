...
We have seen in \ref{Chapter 2} that the j.p.d.f. of eigenvalues of certain random matrices define determinantal point processes. Not only that, but it has the same correlation functions as another point process: The so called \textit{Coulomb gas}. These can be though of as a physical gas of repelling ions restricted to some conductor under a confining potential. With this interpretation one can think of what happens to these gases in the \textit{thermodynamic limit}. Under the appropriate conditions that will be developed in this chapter, these particles are known to converge to a limit equilibrium configuration. Physically, this is the regime we are used to work with; in thermodynamics we often study macroscopic properties of gases such as \textit{entropy} or \textit{temperature}. In our matrices, these equilibrium configurations will refer to the eigenvalue univariate density function and, as mentioned, expressed by the correlation kernel.

In this chapter we will expand on the usual formulation of minimization problems for the system energy. We begin discussing \textit{equilibrium measures} of eigenvalues - the unique measure that minimizes our functional problem. This type of problem can be solved by considering the \textit{Euler-Lagrange}, or \textit{Variational}, \textit{Equations}. These give us a way to calculate the equilibrium measures for the ensemble of random matrices we treat in \autoref{chapter: planarensembles}. Moreover, we finish this chapter doing some informal calculations that intend to motivate the asymptotic expression we get for the partition function on \autoref{chapter: planarensembles}. This also gives a basis estimate to which we compare the order of approximation obtained in \citeonline{Byun_2023} and explored in \autoref{chapter: planarensembles}. 

We based \autoref{Section: Equilibrium}, on equilibrium measures, on the works of \citeonline{saff2024logarithmic} and \citeonline{deiftorthogonal}. The argument to derive an expression for the free energy in \autoref{Section: Free eenrgy} is presented in \citeonline{Livan_2018}. We will explore the concept of equilibrium measures for these particle systems and state some important results relevant to computations that follows.

\section{Equilibrium Measures}
\label{Section: Equilibrium}

The description of the eigenvalues as a \textit{Coulomb gas} discussed in \autoref{Section: Coulomb Gas} and as a \textit{point process} in \autoref{Section: RMT} raises some ways to think of this system of particles. Especially, when dealing with the large $n$ limit, the physical interpretation of this system as a interacting gas lead us to the idea of minimizing the energy of the system. In such minimization the particles should find themselves in what we call an equilibrium configuration  $\xi = \{\mmany{x}{n}\}$. In the large $n$ limit, under some mild conditions to be discussed, the counting measure defined with respect to this configuration converges to a continuous measure with compact support. This limit object is what we call an \textit{equilibrium measure}. To clarify the point, suppose we are dealing with the j.p.d.f.
...
This logarithmic potential can be easily interpreted if one reconsiders the Coulomb gas analogy discussed in \autoref{Section: Coulomb Gas}. Note that the interaction between two particles of gas we considered is logarithmic. Then, taking the empirical counting measure $\delta_\Lambda = 1/n \sum_{i=1}^n \delta_{\lambda_i}$ introduced in \eqref{Equation: empirical measure} we can write
\begin{equation}
	U^{\delta_\Lambda}(z) = \int \ln \frac{1}{|z - t|} \dd \delta_\Lambda(t) = \sum_{i=1}^n \ln \frac{1}{|z  - \lambda_i|}.
\end{equation}
This is precisely the potential energy exerted by the particles on the point $z$. There are a few useful properties one can extract about this quantity.

\begin{proposicao} \cite{saff2024logarithmic}
	Let $\mu$ be a finite positive Borel measure of compact support on the complex plane $\C$. Then the logarithmic potential is super-harmonic on the whole plane, that is, it have the following properties.
	\begin{enumerate}
...
For technical reasons we also state \autoref{Lema: positive kernel} regarding the logarithmic energy that will become useful in a later result although we do not to prove it.
...
If the energy $V$ is finite, there is a unique measure $\mu = \mu_\Sigma \in \M(\Sigma)$ for which the minimum energy $V = \inf\{ I(\mu) | \mu \in \M(\Sigma)\}$ is reachable. \cite{saff2024logarithmic} Such a measure $\mu_\Sigma$ is called the \textit{equilibrium distribution} or \textit{equilibrium measure} of the compact set $\Sigma$. These concepts have a natural electrostatic interpretation. Suppose $\Sigma$ is a conductor and charged particles are placed on it. If the force between the particles is proportional to the reciprocal of their distance, then $\mu_\Sigma$ is their equilibrium configuration.


% For such a function we have that its logarithm potential is always smaller or equal to the energy $I_\Sigma$. 
%\begin{definicao}
...
%\end{equation} 
We should at this point reintroduce the external potential $Q$. Surely an external force will change the minimum energy state and equilibrium configurations. For this case, a weighted version of the problem was developed. The main difference now is that $\Sigma$ is not required to be compact and it is imposed that there is no positive mass around infinity. There is a natural way to enforce these conditions. Let $\Sigma \subset \C$ be a closed set and $\wf : \Sigma \rightarrow [0, \infty)$. This function $\wf$ is called a \textit{weight function} on $\Sigma$. Certain weight functions make it so that there is no positive mass around infinity; we call such function \textit{admissible}.

\begin{definicao}	\label{Definition: Weight Admissibility}
	Define also the \textit{logarithmic capacity} to be	$\cpct{(\Sigma)} = \ee^{-V}$, $V = \inf\{ I(\mu) | \mu \in \M(\Sigma)\}$. A weight function $\wf$ is said to be \textbf{admissible} is it satisfies the following three conditions
	\begin{enumerate}
...
\begin{exemplo}
	(Classical weights) The Gaussian ensemble weight function is
		\begin{equation}
		\wf(\lambda) = \ee^{-\lambda^2}.
	\end{equation}
	We say this is an admissible weight function, ot do so we must prove that it satisfies all three conditions in \autoref{Definition: Weight Admissibility.} Firstly, it is positive continuous functions in $\R$. Because its positive, $\Sigma_0 = \R$. Since there is a 
	For the second condition, $I(\wf)$ is finite such that the energy $V$ is positive.
...
\begin{equation}
	\lim_{|z| \rightarrow \infty, z \in \Sigma} \{ Q(z) - 2 \ln |z| \} = \infty.
\end{equation}
...
\begin{align}
	I^\wf(\mu) & = \int \int \log \left[ |t - s| \wf(t) \wf(s) \right]^{-1} \mu(t) \dd \mu(s), \\
	& = \int \int \ln |t - s|^{-1} \dd \mu(t) \dd \mu(s) + \int \Q(s) \dd  \mu(s), \\
	& = \int U^\mu(s)  \dd \mu(s) + \int \Q(s) \dd  \mu(s),
\end{align}
is recovered if both integrals exist and are finite. 

On the next section we move on to the matter of existence of an \textit{equilibrium measure} and unicity of $\mu^{\wf}$ for a given external field $\Q$ given by some polynomial
...
\begin{exemplo}
	Take the Coulomb gas measure described in \autoref{Section: Coulomb Gas}, restricted to the real line under the external potential
	\begin{equation}
		Q(x) = t x^{2m}.
	\end{equation}
	By finding configurations that minimizes the Hamiltonian, one also finds the more likely configurations. These configurations, when $n \rightarrow \infty$ converges to a measure $\mu^*_Q$ that minimizes the logarithmic energy. It can be shown that this measure is given by
	\begin{equation}
...
\begin{teorema} (Variational Equations)
	The equilibrium measure $\mu^\wf$ for the given variational problem in \autoref{Section: Equilibrium} satisfies the following conditions. There exists a real constant $l$ such that 
	\begin{enumerate}
		\item
		\begin{equation}
...
\begin{proof}
	...
	($\implies$) We recall that $\mu^\wf$ has compact support by \autoref{Theorem: existence equilibrium} and $I^\wf(\mu^\wf) = E^\wf < \infty$. Suppose $\mu^* \in \M^1(\C)$ has compact support and $I^\wf(\mu^*) < \infty$. Then, defining a convex combination
	\begin{equation}
	...
	\end{multline}
	Each term on the right is finite since $\mu^\wf, \mu^*$ have compact support and $I^\wf (\mu^\wf), I^\wf(\mu^*) < \infty$. As $I^\wf(\mu_t) \geq I^\wf (\mu^\wf)$ for $0 \leq t \leq 1$ by assumption of minimality and \autoref{Lema: positive kernel}, is is necessary that
	\begin{equation}
...
\begin{equation}
	\dd \mu^\wf(z) = \phi(z) \dd A 
	\label{Equation: eqmeasure density}
\end{equation}
for some continuous function $\phi$ of compact support and $\dd A = \dd^2 z/\pi$ the area measure, then
%\begin{equation}
...
In this section we will calculate the equilibrium measure for the \textit{random normal ensemble of matrices}.  This is especially helpful as in \autoref{chapter: planarensembles} we will need this measure to perform asymptotic calculation on its partition function. 
...
We want to determine the support of the equilibrium measure for the eigenvalues of the planar normal ensembles.
...
\end{teorema}
\begin{proof}
	We will start on the \textit{ansatz} that this support has two profiles; it can either be a disk centered at the origin with radius $r_1$ denoted by $D(r_1)$ or it can be an annulus centered at the origin with internal radius $r_0$ and external radius $r_1$ denoted by $A(r_0, r_1)$. We have the following theorem. 
	
	Let $\mu = \frac{1}{4\pi} (r q'(r))' \Ind_{\mathcal{S}} \in  \M^1(\C)$. We want that 
	\begin{equation}
		\int_{\C} \dd \mu = 1,
	\end{equation}
	that is, 
	...
	\begin{equation}
		...
		\end{cases}
	\end{equation}
	By the definition for $r_0, r_1$, $2 U^{\mu}(z) \geq l - q(|z|)$ for $z \in \C$ but  $2 U^{\mu}(z) = l - mq(|z|)$ for $r_0 \leq |z| \leq r_1$. Therefore, $\mu_Q = \mu$ and we have found our equilibrium measure.
\end{proof}
...
\end{equation}
Obtaining a expression for the Free Energy rigorously is the main goal for \autoref{chapter: planarensembles}. 