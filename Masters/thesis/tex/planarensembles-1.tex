We are going to discuss in this chapter the ideas developed in the paper \citeonline{Byun_2023}, where asymptotics of partition functions were developed for a large class of random matrix ensembles taking advantage of the orthogonal polynomials present in these models. By the end, we hope to clarify with mathematical rigor the origin for the limit-behavior expression for the \textit{Partition Function} $\pZ{n}{Q}$ given by
\begin{equation}
...
Given some measure $p_n (z_1, \cdots, z_n)$, one can think of its norming function. This is what is called here the partition function $\pZ{n}{Q}$. Its logarithm represents the free energy of the system and is a quantity of interest in physics and mathematics. To get its asymptotics we will perform the following steps.

\begin{itemize}
	\item \textit{Partition functions can be expressed in terms of the norming constants}:  Remind \autoref{Theorem: Kernel} and \eqref{Equation: univariate kernel} that stated that normal ensembles' eigenvalues univariate density can be expressed as a Kernel; a function of the orthogonal polynomial with respect to the weight function $\wf$. An import result for normal ensembles gives the partition function as a function of the product of norming constants $h_j \deff h_{j,n}$. Then, as in \ref{Theorem: Kernel}, we can write the free energy as
	\begin{equation}
			\ln \pZ{n}{Q} = \ln{n!} + \sum_{j=0}^{n-1} \ln{h_j}.
	\end{equation}
	\item \textit{Asymptotics of norming constant using the asymptotic Laplace technique}: If we set the weight function as $\wf(z) = \ee^{-nQ(z)}$ for $Q$ a radially symmetric potential, i.e. $Q(z) = q(|z|)$, the integral expression for the norming constants can be rewritten as a real line integral and approximated by the Laplace technique.
	\begin{align}
			h_j 
			& = \int_{\C} |z|^{2j} \ee^{-n Q(z)} \dd^2 z, \\
			& =  \int_{\R_+} 2 r \ee^{-n \left( q(r) - \frac{2j}{n} \ln(r) \right)} \dd r, \\
			&= \int_{\R_+} 2r \ee^{-n V_{\tau(j)}(r)} \dd r \approx \ee^{-n V_{\tau(j)}(r_c(\tau(j)))}.
	\end{align}
	\item \textit{Asymptotic of the summation on $\ln h_j$ using quadrature}: Finally, to approximate the summation on $\ln(h_j)$ we use the previously discussed idea of quadrature in \autoref{section: quadrature} relating the limit summation with the corresponding integral
	\begin{equation}
...
\subsection{Norming Constants}

We must study the asymptotic behavior of the norming constant $h_j$ described in \autoref{Theorem: Kernel}. We expand on the idea in \autoref{Section: outline asymptotic} where we stated that the integral could be approximated by its value in a critical point. This is, of course, based on the Laplace technique described in \autoref{subsection: Laplace}. Defining $z = r \ee^{\ii \theta}$ ($|z| = r$), because of the symmetry on $Q$ we write
\begin{align}
...
\begin{align}
	h_j & = \int_{\R_+} 2r \ee^{-n V_{\tau(j)}(r)} \dd r, \\
	&  \approx \ee^{-n V_{\tau(j)}(r_c(\tau(j))}.
	\label{Equation: Asymptotic for norming constant}
\end{align}

\subsection{Approximating the Summation}

Now, we remember that the expression for the free energy, accordingly to \autoref{Theorem: Kernel} is given in terms of the norming constants by
\begin{equation}
	\ln \pZ{n}{Q} = \ln{n!} + \sum_{j=0}^{n-1} \ln{h_j}.
\end{equation}
...
\begin{equation}
	\ln \pZ{n}{Q} = \ln{n!} - n \sum_{j=0}^{n-1} V_{\tau(j)}(r_c(\tau(j))).
\end{equation}

Performing this sum is not a clear procedure at first. One way to do it is to  reconsider the quadrature idea described in \autoref{section: quadrature}. With this, we can rewrite our summation as an integral, which may be easier to solve. In our case, the summation can be approximated as
\begin{equation}
...
\begin{lema}	\label{Lema: diff r tau}
	The differential of $r$ as a function of $\tau$ at $r(\tau)$ is given by
	\begin{equation}
...
\begin{proof}
	...
	\begin{equation}
		\left. \frac{2}{(r q'(r))'} \right|_{r = r(\tau)} =   \frac{2}{q'(r(\tau)) + r(\tau)q''(r(\tau))} =   \frac{2}{\frac{2\tau}{r(\tau)} + r(\tau)q''(r(\tau))}  =  \frac{2}{r(\tau) V^{(2)}_\tau(r(\tau))}.
	\end{equation}
\end{proof}
...
\end{equation}
This lead us to the main term in the expression  \eqref{Equation: asymptotic for free energy}, with order $n^2$. Note that this does not depend on the cases for $r_0$ bigger or equal to $0$. This difference only appears in lower order approximations. This can be seen if one considers a more complete approximation and write
\begin{align}
		n \sum_{j=0}^{n-1} V_{\tau(j)}(r_{\tau(j)}) = & n^2  \int_{r_0}^{r_1} V(r) \frac{(r q'(r))'}{2} \dd r \\ & - \frac{n}{2} \left( V_{\tau(n)}(r_{\tau(n)})  - V_{\tau(0)}(r_{\tau(0)}) \right)  \\ & + \frac{n}{12} \left[ \partial_t V_{\tau(t)}(r_{\tau(t)}) |_{t=n} - \partial_t V_{\tau(t)}(r_{\tau(t)}) |_{t=0} \right] \\
		& + \Boh(n^{-3}).
\end{align}
...
\subsection{Asymptotics of the Norming Constant}

For this subsection, we will focus on getting the asymptotic for the integral
\begin{equation}
	h_j = \int_{\R_+} 2r \ee^{-n V_{\tau(j)}(r)} \dd r
\end{equation}
for each of the degrees $j$. With this, we can proceed to examine the summation over $h_j$ needed top evaluate the free energy as it was shown in \autoref{Theorem: Kernel}.
...
\begin{proof}
	...
	\begin{align}
		...
		& = \delta_n^2 c + \Boh(n^{-1}) + \Boh(\delta_n^3), \\
		& \geq  \delta_n^2 c.
	\end{align}
...
\subsection{Asymptotics of the Summation}

We return to \autoref{Theorem: Kernel} and turn to the calculation of each term of $	\sum_{j=0}^{n-1} \ln{h_j}$ using the logarithm of the expansion calculated previously in \autoref{Theorem: hj annulus}
\begin{equation}
...
\begin{equation}
	h_j = \int_{\R_+} 2r \ee^{-n V_{\tau(j)}(r)} \dd r
\end{equation}
for each of the degrees $j$. With this, we can proceed to examine the summation over $h_j$ needed top evaluate the free energy. For this case however there is one complication. One should distinguish the following two cases depending on a small constant $1/5 > \epsilon > 0$. 
...
\begin{lema} \label{Lemma: mn - N value contributing}
	Consider the case $m_n \leq j \leq n-1$. For the value-contributing integral we have the asymptotic of $n$ given by
	\begin{equation}
...
So now we need to return to  \autoref{Theorem: Kernel} and calculate each term for $	\sum_{j=0}^{n-1} \ln{h_j}$ using the expansion in terms of \autoref{Theorem: hj disk j>mn} and \autoref{Theorem: hj disk j<mn} for the expression
...
To prove this theorem, we must give the asymptotics of both summations. This means we need both \autoref{Lemma: Summation disk <mn}, referring to the sum from powers $0$ to $m_n-1$, and \autoref{Lemma: Summation disk >mn}, referring to the sum from $m_n$ to $n-1$. We start by the lower power terms.
