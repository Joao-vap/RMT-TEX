...
\begin{frame}
	...
	\only<1-4>{
		\begin{equation*}
			p_n(\{\lambda_i\}_{i=1}^n) = p\left( \eigenvalues \right) = \frac{1}{\pZ{n}{Q}} \prod_ {i < j}^n |z_i - z_j|^2 \prod_{j=i}^{n} \ee^{- n \Q(z_i)} 
		\end{equation*}
	}
	\only<1-4>{
		\visible<2-4>{
...
\begin{frame}{Point Process}
	...
	\vspace{-0.9cm}
	\end{center}	
	\visible<2->{Define a quantity \only<2,6->{$\xi(A)$}\only<3>{$\xi(\textcolor{red}{A})$}\only<4>{$\xi(\textcolor{blue}{A})$}\only<5>{$\xi(\textcolor{teal}{A})$} s.t.
	\begin{equation*}
		\only<2,6->{\xi(A)}\only<3>{\xi(\textcolor{red}{A})}\only<4>{\xi(\textcolor{blue}{A})}\only<5>{\xi(\textcolor{teal}{A})}=\sum_{k=1}^{n}\delta_{\only<3>{\textcolor{red}{A}}\only<4>{\textcolor{blue}{A}}\only<5>{\textcolor{teal}{A}}}(z_k)\only<3>{=\textcolor{red}{21}}\only<4>{=\textcolor{blue}{9}}\only<5>{=\textcolor{teal}{10}}
	\end{equation*}
	\visible<6->{
...
\begin{frame}
	...
	\end{column}
		...
		\begin{equation*}
			\only<2->{\only<2,6->{\xi}\only<3>{\xi^{(1)}}\only<4>{\xi^{(2)}}\only<5>{\xi^{(3)}} (\textcolor{red}{A}) = \sum_{k=1}^{n} \delta_{\textcolor{red}{A}}(\only<2,6->{z_k}\only<3>{z^{(1)}_k}\only<4>{z^{(2)}_k}\only<5>{z^{(3)}_k})}
		\end{equation*}
	\end{column}
	\end{columns}
	\visible<6->{
	What is
	\begin{equation*}
		\mathfrak{p}(A) = \E[\xi(A)] \deff \E[\#(\chi_n \cap A)], \quad A \in \mathcal{E}.
	\end{equation*} 
	\visible<7->{We can assume the existence of a density $\p_1(z)$. \visible<8->{Then
		\begin{equation*}
			\mathfrak{p}(A) = \int_A \p_1(z) \dd^2 z
		\end{equation*}
		where $\dd^2 z$ is the area measure.} \visible<9->{The \textbf{$1$-point correlation function} $\p_1$  represents the infinitesimal probability of a point of the process being at a location $z$, i.e. $\mathbb{P}(\xi(\dd^2 z) = 1) = \p_1(z) \dd^2 z$.}
	}
	}
\end{frame}
...
\begin{frame}[allowframebreaks]
	...
	\end{equation}
	but $r_0$ is the largest solution of $r q'(r) = 0$ and $r_ 1$ is the smallest solution of $r q'(r) = 2$. Now, by Euler-Lagrange Theorem, if $\mu$ satisfies that
	\begin{align}
	...
	\begin{equation}
		...
		\end{cases}
	\end{equation}
	With this, we calculate that
	\begin{equation*}
		U^{\mu}(z) = \frac{1}{4\pi} \int_{r_0}^{r_1} (r q'(r))'  \int_{-\pi}^{\pi} \ln \frac{1}{|z - r\ee^{\ii \phi}|} \dd \phi \dd r.
	\end{equation*}
	And, for $|z| < r_0$
	\begin{equation*}
