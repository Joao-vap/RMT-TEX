...
\begin{frame}
	...
	\end{equation*} 
	\visible<7->{We can assume the existence of a function $\p_1(z)$ s.t.
		\begin{equation*}
...
\begin{frame}
	\begin{theorem} \label{Theorem: Euler-Lagrange} (Strong Variational Equations)
		...
		We define $U^\mu(z) =  \int \ln{|z-t|^{-1}} \dd \mu_Q(t)$.
	\end{theorem} 
\end{frame}
...
\begin{frame}
	...
	\visible<9->{
		\begin{columns}
			...
			\end{column}
			\begin{column}{0.5\textwidth}
				$r_0$ is the biggest $r$ s.t. $q'(r_0) = 0$. 
				\begin{align*}
					 q(r_0) \leq q(|z|), \quad |z| < r_0.
				\end{align*}
				$r_1$ is the smallest $r$ s.t. $r_1 q'(r_1) = 2$. Then,  for $|z| > r_1$ 
				\begin{align*}
