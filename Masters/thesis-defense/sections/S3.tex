
{
	...
	\section{Asymptotics of the Partition Function}
}
...
\begin{frame}{Quick recap}
	...
	\visible<8>{
	\begin{equation*}
		\ln(\pZ{n}{Q}) = -I_Q n^2+\frac{1}{2} n\ln n+E_Q n+G\ln n+F_Q + o(1), \quad n \rightarrow \infty.
		\label{Equation: asymptotic for free energy}
	\end{equation*}
	Result derived in Byun, Sung-Soo and Kang, Nam-Gyu and Seo, Seong-Mi article.
	}
	}
\end{frame}
...
\begin{frame}{Determinantal P.P.}
	...
	\begin{equation*}
		...
		\only<5-7>{
			p_n(\mmany{z}{n}) = \frac{\prod_{l=1}^{n}h_l}{\pZ{n}{Q}} \det\left[ K_n(z_i, z_j) \right]_{n\times n} 
		}
	\end{equation*}
	\only<1>{\vspace{2cm}}
	\only<1-5>{
		...
		\visible<2-5>{
			...
			\only<2>{
				\begin{equation*}
					\prod_{i<j}^{n} |z_i - z_j|^2 = \det\left(\sum_{l=1}^{n} \overline{q_{l-1}(z_i)} q_{l-1}(z_j) \right)_{n\times n}
				\end{equation*}
			}
			\only<3>{
				\begin{equation*}
					\frac{\prod_{i=1}^{n}\wf(z_i)}{\prod_{l=1}^{n}h_l }\prod_{i<j}^{n} |z_i - z_j|^2 = \det\left(\sqrt{\wf(z_i)\wf(z_j)}\sum_{l=1}^{n} \frac{1}{h_l} \overline{q_{l-1}(z_i)} q_{l-1}(z_j) \right)_{n\times n}
				\end{equation*}
			}
			\only<4->{
				\begin{equation*}
					\frac{\wf(z_i)}{\prod_{l=1}^{n}h_l } \prod_{i<j}^{n} |z_i - z_j|^2 = \det\left( K_n(z_i, z_j) \right)_{n\times n}
				\end{equation*}
			}
	...
	\only<6>{
		\vspace{-0.4cm}
		\begin{theorem}
			Let $\{q_j\}_{j=0}^\infty$ be a family of monic polynomials such that each $q_j(\lambda)$ is of degree $j$. Impose also that exists finite positive constants $\{h_{j,n}\}_{j=0}^\infty$ s.t.
			\begin{equation*}
				\langle q_j, q_k \rangle_{\wf} = \int_{\C} q_j(z) \overline{q_k(z)} \ee^{-nQ(z)} \dd^2 z =h_j \delta_{\{j=k\}}
			\end{equation*}	
			for any $j,k>0$. Then, $K_n(x,y)$ has the reproducing property, i.e.
			\begin{equation*}
	...
	\only<7>{
		\begin{colorblock}{Corollary}{bg=Mahogany,fg=white}
			...
			\begin{equation*}
				\pZ{n}{Q} = \prod_{l=1}^nh_l  \int_{\C^n} \det\left[ K_n(\lambda_i, \lambda_j) \right]_{i,j=1}^{n} \prod_{i=1}^{n} \dd^2 \lambda_i = n! \prod_{l=1}^nh_l
			\end{equation*}
		\end{colorblock}
	}
\end{frame}
...
\begin{frame}{Norming constants}
	...
	\end{equation*} 
	\visible<2->{
	Notice that, because of radial symmetry
	\begin{align*}
...
%	We start by approximating $V_{\tau(j)}$ around its global minimum by Taylor's theorem 
%	\begin{equation*}
...
%	\begin{equation*}
%	\!\!	\int_{r^*-\frac{\ln(n)}{\sqrt{n}}}^{r^*+\frac{\ln(n)}{\sqrt{n}}} 2s \ee^{-n V_{\tau(j)}(s)} \dd s \approx \frac{\ee^{-n  V_{\tau(j)}(r^*)}}{\sqrt{n}} \int_{-\ln(n)}^{\ln(n)} 2 r^* \ee^{-\frac{1}{2} |V^{(2)}_{\tau(j)}(r^*)|r^2} \dd r
%	\end{equation*}
%	and
%	\begin{equation*}
%	\!\!	\int_{r^*+ \frac{\ln(n)}{\sqrt{n}}}^{\infty} |2s| \ee^{-n V_{\tau(j)}(s)} \dd(s) = \frac{\ee^{-n V_{\tau(j)}(r^*)}}{\sqrt{n}} \Boh(n^{-2})
%	\end{equation*}
%\end{frame}
...
We begin with $\Delta_n  = (b - a)/n$, s.t.
\begin{align*}
	\int_{a}^{b} f(x) \dd x & \approx \Delta_n \sum_{j = 0}^{n} \frac{f(a + j \Delta_n) + f(a + (j+1) \Delta_n)}{2}, \\
	& = \Delta_n \left[ \frac{f(a)}{2} + f(a+\Delta_n) + \cdots + f(b-\Delta_n) + \frac{f(b)}{2} \right], \\
	& = \Delta_n \sum_{j = 1}^{n-1} f(x_j^*) + \frac{\Delta_n}{2} \left( f(b) + f(a) \right), \\
	& = \Delta_n \sum_{j = 0}^{n-1} f(x_j^*) + \frac{\Delta_n}{2} \left( f(b) - f(a) \right).
\end{align*}
\end{frame}

\begin{frame}
	\begin{theorem} (Euler-Maclaurin Formula)
		For some function $f \in C^p(\R)$ and $p \geq 1$ let $\Delta_n = (b - a)/n$. Then, the Euler-Maclaurin formula for the interval $[a,b]$ is given by
		\small
		\begin{multline*}
		\!\!\!\!	\Delta_n \sum_{j=1}^{n-1} f(a + j \Delta_n) = \int_{a}^{b} f(x) \dd x - \frac{\Delta_n}{2} (f(b) - f(a)) + \sum_{r = 1}^{p-1} \frac{\Delta_n^{r+1} B_{r+1}}{(r+1)!} \left[ f^{(r)}(b) - f^{(r)}(a) \right] + R_p
		\end{multline*}
		\normalsize
	\end{theorem}
		\begin{center}
