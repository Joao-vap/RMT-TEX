...
\begin{frame}{Quick recap}
	...
	\begin{columns}
		\begin{column}{0.5\textwidth}
			...
			\only<2-3>{
			\begin{equation*}
				p_n(\{\lambda_i\}_{i=1}^n) = \frac{1}{\pZ{n}{Q}} \prod_ {i < j}^n |z_i - z_j|^2 \prod_{j=i}^{n} \ee^{- n \Q(z_i)} 
			\end{equation*}
			}
			\only<4-5>{
			\begin{align*}
					& p_n(\{\lambda_i\}_{i=1}^n) = \frac{\ee^{-n^2 \Hf(\chi_n)}}{\pZ{n}{Q}}, \quad \text{with}\\
		...
		\begin{column}{0.5\textwidth}
			...
			\begin{table}
				...
				\begin{tabular}{l | c | c }
					$\eP(\m{X})$ & $\Se$ & $\M_n$ \\
					\hline \hline
					$\ee^{-n \Tr(\m{M}^2)}$ &$\R$  & General\\ 
					$\ee^{-n \Tr{(\m{X}^* \m{X})}}$ &  \cellcolor{red!10}$\C$ &  Hermitian \\
					\cellcolor{blue!10}$\ee^{-n \Tr{(Q(\m{X}))}}$ & $\mathbb{H}$ & \cellcolor{green!10}Normal \\
					\vdots  & \vdots  &  \vdots \\
					\hline
...
\begin{frame}{Determinantal P.P.}
	...
	\only<1-5>{
		...
		\visible<2-5>{
			...
			where $q_i$ form a polynomial basis to the space.
		}
	}
	\only<6>{
		\vspace{-0.4cm}
		\begin{theorem}
			...
			\begin{equation*}
				\langle q_{j,n}, q_{k,n} \rangle_n = \int_{\C} q_{j,n}(z) \overline{q_{k,n}(z)} \ee^{-nQ(z)} \dd^2 z = h_{j,n} \delta_{\{j=k\}}
			\end{equation*}	
...
\begin{frame}
	...
	\visible<5>{
		\begin{equation}
		\lim_{\lambda \rightarrow \infty} \frac{\ee^{-\lambda(f(r^*))}}{\ee^{-\lambda(f(r^* - \epsilon))}} = \lim_{\lambda \rightarrow \infty} \ee^{-\lambda(f(r^*) - f(r^* - \epsilon))} = \lim_{\lambda \rightarrow \infty} \ee^{\lambda |c|} \rightarrow + \infty. 
		\end{equation}
	}
\end{frame}

\begin{frame}{Laplace Technique}
	We start by approximating $V_{\tau(j)}$ around its global maximum by Taylor's theorem 
	\begin{equation*}
	...
	\begin{equation*}
		\int_{r^*-\frac{\ln(n)}{\sqrt{n}}}^{r^*+\frac{\ln(n)}{\sqrt{n}}} 2s \ee^{-n V_{\tau(j)}(s)} \dd s = \frac{\ee^{-n  V_{\tau(j)}(r^*)}}{\sqrt{n}} \int_{-\ln(n)}^{\ln(n)} 2 \left(r^* + \frac{r}{\sqrt{n}}\right) \ee^{-\frac{1}{2} |V^{(2)}_{\tau(j)}(r^*)|r^2 - \epsilon \left(\frac{r^3}{\sqrt{n}}\right)} \dd r.
	\end{equation*}
	and
	\begin{equation*}
		\int_{x^*+ \frac{\ln(n)}{\sqrt{n}}}^{\infty} |2s| \ee^{-n V_{\tau(j)}(s)} \dd(s) \leq \ee^{-n V_{\tau(j)}(r^*+ \ln(n)/\sqrt{n})} \normL{1}{g}{}
	\end{equation*}
\end{frame}
...
\begin{frame}
	...
	\end{equation}
	with a finite and $g \in L^1(a,b)$. Assume that $\phi$ has a unique global minimum at $t=c \in (a,b)$. In addition, suppose that for some $\delta > 0$ and some $\alpha > -1$, the function $g$ takes the form
	$
		g(t) = (t-a)^{\alpha} g_0(t)
	$
...
\begin{align*}
	...
	& = h \sum_{j = 0}^{n} \frac{f(x_{i}) + f(x_{i-1})}{2}, \\
