...
We are going to discuss in this chapter the ideas involved in the paper \cite{Byun_2023}, where asymptotics were develop through orthogonal polynomials. To introduce the ideas discussed in the paper, we first introduce the models that are dealt with: Coulomb Gases in the complex plane (for $\beta = 2$: \textit{random normal matrix} and \textit{planar symplectic ensembles}). They discuss radially symmetric potentials, but specially for the quadratic potential, in the $2-\beta$ regime, these ensembles should be correspondent to the \textit{complex} and \textit{planar} Ginibre ensembles. Then we go through the ideas involved in the asymptotic for the orthogonal polynomial and finally into the techniques used to control the summations and final results.

\input{./Text/PaperContext.tex}

\input{./Text/OrthogonalAsymptotics.tex}

\input{./Text/SummationEstimation.tex}

\input{./Text/Generally.tex}
