...
\begin{claim}
	Let $\m{V}$ be a matrix with entries $\{V_{i,j}\} = v_i^{j-1}$ such that its determinant is given by $\prod_{i < j}^{n} |v_j - v_i|$. Then, if we define $\m{V}'$ with $\{V_{i,j}\} = p(v_i)$ with $p$ a monomial with degree $j$.
	\label{Prop: Vandermonde polynomial}
\end{claim} 