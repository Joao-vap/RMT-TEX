...
The scale and moment assumption taken are not as unnatural as they may seem at first. To proposicaoerly understand this definition, note that if $\mmany{\lambda^2}{n}$ are the ordered eigenvalues of $\{Y_n\}_{i,j}$, we can estimate that 
\begin{equation}
...
\begin{obs}
	We have focused on random matrix ensembles distributed according to p.d.f.s $\eP$ defined with respect to the flat Lebesgue measure induced by the canonical embedding of $\eS$ into Euclidean space. Note here, which was not obvious for me, that the measures $\nu$ we define are given in terms of the p.d.f.s $\eP$ and the Lebesgue measure $\pe$ by Radom-Nikodin \begin{equation}\nu(A) = \int_{A} \eP \dd \pe,\end{equation} or more simply, $\dd \nu = \eP \dd \pe$.
\end{obs}

\begin{teorema}
	(Haar, compact case). Given a compact group $\eS$, let $U \in \eS$ be a generic variable. There exists a unique (up to normalization measure) $\dd \pe(U)$ on $\eS$, called the \textbf{Haar measure} such that \begin{equation}\int_{\eS} \dd \pe(U) < \infty\end{equation} and for any fixed $U_0 \in \eS$, $$\dd\pe(U_0U) = \dd\pe(U) = \dd\pe(UU_0),$$ that is, finite for compact sets and invariant by left and right actions. If $\dd \pe(U)$ integrates to unity in $\eS$ it is referred as the \textbf{Haar probability measure}.
\end{teorema}
...
\begin{proposicao}
	...
	\end{equation}
	\label{proposicao: Gaussian, Laguerre, Jacobi}
\end{prop}
