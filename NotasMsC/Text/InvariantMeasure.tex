...
\begin{prop}
	...
	\begin{equation}
		\dd \m{X} = |\Delta_n|^{\beta} \dd \m{\Lambda} \dd \mu_{Haar}(\m{U})
	\end{equation}
	...
	\begin{equation}
		\dd \mu_{Haar}(\m{U}) = \bigwedge_{1\leq l < k \leq n} \bigwedge_{s=1}^\beta \left( \m{U}^{\dagger} [\dd U_{ij}]_{i,j=1}^n \right)_{lk}^{(s)}
	\end{equation}
...
\begin{equation}
	\dd \m{G} = \J(\lambda) \dd \m{\Lambda} \dd \mu_{Tri}(\m{T}) \dd \mu_{Haar}(\m{U})
\end{equation}
where $\dd \m{\Lambda}$ is the Lebesgue measure on the space housing the eigenvalues, $\dd \mu_{Haar}(\m{U})$ is the Haar measure defined before, $J(\lambda)$ is a Jacobian consisting of products of Vandermonde determinants involving the eigenvalues, and $\mu_{Tri}(\m{T})$ is some measure dependent on the entries of T. From here, it follows that the Lebesgue measure on $\m{G}$ is invariant under the desired conjugation action.
...
\begin{equation}
	\begin{split}
		\eP(\m{X}) \dd \m{X} & = \eP(X_{1,1}, \cdots , X_{n,n}) \prod_{i,j} \dd M_{i,j} \\
...
where, of course,
\begin{equation}
 \vol(\mathcal{G}(n)/\mathcal{G}(1)^n) = \int_{\mathcal{G}(n)/\mathcal{G}(1)^n} \dd \mu_{Haar}(\m{U}).
\end{equation}
Now, with $\wf(\lambda)$ the appropriate classical weight, we can check that
...
\end{equation}
If we have a sequence of matrices $\{\m{M}_n\}$ we say that their spectral measure converges in expectation to a limiting measure $\mu$ if the sequence $\{\E(\mu_{M_n})\}$ converges weakly to $\mu$, that is, if
\begin{equation}
...
\begin{equation}
	\int (z^k + c_{k-1} z^{k-1} + \cdots + c_{0}) \dd \mu_{M} = \frac{1}{n} \sum_{i=1}^n (\lambda_i^k + c_{k-1} \lambda_i^{k-1} + \cdots + c_{0}) = \frac{1}{n} \ Tr(V(\m{M}))
\end{equation}
