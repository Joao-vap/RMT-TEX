...
\begin{enumerate}
	\item Small $x$ values $-2\sqrt{N} < x < 2\sqrt{N}$: For fixed $\gamma \in (0, \pi/2)$ set $x = 2\sqrt{N} \cos{(\gamma)}$ and we obtain
	 \begin{equation}
		\He_n(2\sqrt{N}\cos{(\gamma)}) = \sqrt{\frac{2}{\sin{(\gamma)}}} N^{-\frac{N}{2}} (N-1)^{\left(N - \frac{1}{2}\right)} \ee^{N\left(-3\cos^2{(\gamma)} + \frac{1}{2} \right) + 1} \sin{\left(\theta(\beta_c^+) -N\left(\frac{\sin{(2\gamma)}}{2}+\gamma\right)\right)} 
		(1 + \Boh(N^{-1})),
		\label{Eq: Hermite small asymptotic (n)}
	\end{equation}
	\begin{equation}
		\He_{n-1}(2\sqrt{N}\cos(\gamma)) = \sqrt{\frac{2}{\sin{(\gamma)}}} \frac{(N-2)^{N - \frac{3}{2}}}{(N-1)^{\frac{N-1}{2}}} \frac{\ee^{N\left(-3\cos^2{(\gamma)} + \frac{1}{2} \right) + 1}}{\ee^{\left(3\cos^2{(\gamma)} - \frac{1}{2} \right)}} \sin{\left(\theta(\beta_c^+) -(N-1)\left(\frac{\sin{(2\gamma)}}{2}+\gamma\right)\right)}
		(1 + \Boh(N^{-1})).
	...
	\begin{equation}
		\He_n(2\sqrt{N}\cosh{(\gamma)}) = \ee^{-N\left( \frac{\ee^{-2\gamma}}{2} + \gamma +  1\right) + 1} \frac{(N-1)^{N - \frac{1}{2}} N^{-\frac{N}{2}}}{\sqrt{|1 - \ee^{2\gamma}|}} \left(1 + \Boh(N^{-1}) \right),
		\label{Eq: Hermite large asymptotic (n)}
	\end{equation}
	\begin{equation}
		\He_{n-1}(2\sqrt{N}\cosh{(\gamma)}) = \frac{\ee^{-N\left( \frac{\ee^{-2\gamma}}{2} + \gamma +  1\right)}}{\ee^{-\left( \frac{\ee^{-2\gamma}}{2} + \gamma - 2\right)}} \frac{(N-1)^{-\frac{N-1}{2}}}{\sqrt{|1 - \ee^{2\gamma}|}} 
	...
	\begin{equation}
		\He_n(2\sqrt{N} + N^{-1/6}\xi) = \sqrt{2\pi} (N-1)^{\frac{1}{2}} N^{1/6 - N/2} \ee^{\left(1/2 + N^{-1} - 2\xi \beta_c N^{-2/3} \right)} \Ai(\xi) (1 + \boh(1)),
		\label{Eq: Hermite edge asymptotic (n)}
	\end{equation}
		\begin{equation}
		\He_{n-1}(2\sqrt{N} + N^{-1/6}\xi) = \sqrt{2\pi} (N-2)^{\frac{1}{2}} (N-1)^{4/6 - N/2} \ee^{\left(1/2 + (N-1)^{-1} - 2\xi \beta_c (N-1)^{-2/3} \right)} \Ai(\xi) (1 + \boh(1)).
...
\begin{equation}
 	\begin{split}
 		 ...
 		 (1 + \Boh(N^{-1})) 
 		 \\
 		 & =  C_n^0 \sqrt{\frac{\pi}{N\sin{(\gamma)}}} \ee^{N(-3\cos^2{(\gamma)} - \frac{1}{2})}
 		 \left(
 		 ...
 		 (1 + \Boh(N^{-1})) \\
 		 & = \frac{(N-1)! N^{\frac{1}{2}\left(1-N\right)}}{2\pi \ii} \sqrt{\frac{\pi}{N\sin{(\gamma)}}} \ee^{N(-3\cos^2{(\gamma)} - \frac{1}{2})}
 		 2\ii \sin{
 		 \left(
 		 \theta(\beta_c^+)
 		 ...
 		 \right)} 
 		 (1 + \Boh(N^{-1})) \\
 		 & = \sqrt{\frac{2}{\sin{(\gamma)}}} N^{-\frac{N}{2}} (N-1)^{\left(N - \frac{1}{2}\right)} \ee^{N\left(-3\cos^2{(\gamma)} + \frac{1}{2} \right) + 1} \sin{\left(\theta(\beta_c^+) -N\left(\frac{\sin{(2\gamma)}}{2}+\gamma\right)
 		 \right)} 
 		 (1 + \Boh(N^{-1}))
 	\end{split}
\end{equation}
...
\begin{equation}
	\begin{cases}
		\Re[(-\alpha \beta_c^+ + \frac{1}{2} (\beta_c^+)^2 + \ln{(\beta_c^+)})] = -3\cos^2{(\gamma)} - \frac{1}{2}; \qquad \Im[(-\alpha \beta_c^+ + \frac{1}{2} (\beta_c^+)^2 + \ln{(\beta_c^+)})] = \frac{\sin{(2\gamma)}}{2} + \gamma; \\
		\Re[ \overline{(-\alpha \beta_c^+ + \frac{1}{2} (\beta_c^+)^2 + \ln{(\beta_c^+)})} ] = -3\cos^2{(\gamma)} - \frac{1}{2}; \qquad  \Im[ \overline{(-\alpha \beta_c^+ + \frac{1}{2} (\beta_c^+)^2 + \ln{(\beta_c^+)})}] = -(\frac{\sin{(2\gamma)}}{2} + \gamma).
	\end{cases}
\end{equation}
...
We need now to determine the Steepest Descent Path. In this case is more clear what the path should be the one passing through the critical point $\ee^{-\gamma}$.  Then, repeating the process we did on Case 1 - changing the path of integration - we can make the substitution
\begin{align}
		...
		\left(1 + \Boh(N^{-1}) \right)
		\\
		& =	\ee^{N( - 1) + 1} \frac{(N-1)^{N - \frac{1}{2}} N^{-\frac{N}{2}}}{\sqrt{|1 - \ee^{2\gamma}|}} 
		\left(1 + \Boh(N^{-1}) \right) \\
...
Now, it is clear from the previous cases that when we evaluate the integral, the contributions from the critical point $\beta_c = 1$ should be exponentially bigger - that is, the asymptotic of the integral can be deduces from the values around this point. We know that 
\begin{align}
...
\end{equation}
such that for $\beta = \beta_c + u N^{-\frac{1}{3}}$, for $u$ coming from $\infty \ee^{\ii \frac{4\pi}{3}}$ to $\infty \ee^{\ii \frac{2\pi}{3}}$, we write
\begin{align}
	...
	\int_{\langle}  \ee^{\frac{1}{3} u^3 - \xi u + \Boh(u^4 N^{-\frac{4}{3}})} \dd u
\end{align}
Then we will have, remembering the definition of the Airy function \ref{Def: Airy Function},
\begin{equation}
	\He_n(2N^{\frac{1}{2}} + \xi N^{-\frac{1}{6}}) = \tilde{C}_N \int_{\langle}  \ee^{\left(\frac{u^3}{3} - \xi u\right)} \dd u (1 + \boh(1)) = \tilde{C}_N \Ai(\xi) (1 + \boh(1)),
\end{equation}
for $\tilde{C}_N = (N-1)! N^{\frac{1}{6} - \frac{N}{2}} \ee^{-2\xi\beta_c N^{\frac{1}{3}} + \frac{3N}{2}} = \sqrt{2\pi} (N-1)^{\frac{1}{2}} N^{1/6 - N/2} \ee^{\left(1/2 + N^{-1} - 2\xi \beta_c N^{-2/3} \right)} (1 + \Boh(N^{-1}))$. The result follow from the asymptotic for the Airy function given in \ref{Subsection: Airy Asymptotic}. Again, we can change variables to express
\begin{equation}
	\He_{n-1}(2\sqrt{N} + N^{-1/6}\xi) = \sqrt{2\pi} (N-2)^{\frac{1}{2}} (N-1)^{4/6 - N/2} \ee^{\left(1/2 + (N-1)^{-1} - 2\xi \beta_c (N-1)^{-2/3} \right)} \Ai(\xi) (1 + \boh(1))
\end{equation}
...
%		\\
%		& =  C_n^0 \sqrt{\frac{\pi}{N\sin{(\gamma)}}}
%		\ee^{y\cos{(\gamma)}} \ee^{N(-3\cos^2{(\gamma)} - \frac{1}{2})}
%		\left(
...
%		\\
		...
		\ee^{N\left(-3\cos^2{(\gamma)} + \frac{1}{2} \right) + 1 + y\cos{(\gamma)}} \sin{\left(\theta(\beta_c^+) + y \sin{(\gamma)}-N\left(\frac{\sin{(2\gamma)}}{2}+\gamma\right)\right)} 
		(1 + \Boh(N^{-1}))
	\end{split}
\end{equation}
and for $N-1$,
\begin{equation}
	\He_{n-1}(\Delta) = \sqrt{\frac{2}{\sin{(\gamma)}}} \frac{(N-2)^{N - \frac{3}{2}}}{(N-1)^{\frac{N-1}{2}}} \frac{\ee^{N\left(-3\cos^2{(\gamma)} + \frac{1}{2} \right) + 1 + y\cos{(\gamma)}}}{\ee^{\left(-3\cos^2{(\gamma)} + \frac{1}{2} \right)}} \sin{\left(\theta(\beta_c^+) + y \sin{(\gamma)} - (N-1) \left(\frac{\sin{(2\gamma)}}{2}-\gamma\right)\right)} 
	(1 + \Boh(N^{-1}))
\end{equation}
...
\begin{align}
	...
	& = \left(\frac{\exp{\left( -\frac{1}{2} \left(\sqrt{N}\alpha + N^{-1/2} x \right)^2 \right)}}{\sqrt{2\pi}}  \frac{\exp{\left( -\frac{1}{2} \left(\sqrt{N}\alpha + N^{-1/2} y \right)^2 \right)}}{\sqrt{2\pi}} \right)^{-\frac{1}{2}} \\
	& = \frac{1}{\sqrt{2\pi}} \left( \exp{\left\{ -\frac{1}{2} \left( 4 N \cos{(\gamma)} + 4 x \cos{(\gamma)} + x^2 N^{-1} + 4 N \cos{(\gamma)} + 4 y \cos{(\gamma)} + y^2 N^{-1} \right) \right\}} \right)^{\frac{1}{2}} \\
	& = \frac{1}{\sqrt{2\pi}} \ee^{-2N\cos{(\gamma)}}
	\exp{\left\{ -\left( 2 \cos{(\gamma)} (x+y) +  \frac{1}{2N} (x^2 + y^2) \right) \right\}} 
\end{align}
Note that
\begin{equation}
...
\begin{align}
	K_n^{(2)}(\Delta_x,\Delta_y) & = W_{(N,x,y)} N^{1/2} C \ee^{\frac{1}{2}\sin{(2\gamma)}(x + y)} \sin{\left(\frac{\sin{(2\gamma)}}{2}-\gamma\right)} \frac{\sin{(x - y)}}{x-y}
\end{align}
where
\begin{align}
	C & = \left(\sqrt{\frac{2}{\sin{(\gamma)}}} N^{-\frac{N}{2}} (N-1)^{\left(N - \frac{1}{2}\right)}
	\ee^{N\left(-3\cos^2{(\gamma)} + \frac{1}{2} \right) + 1} \right) \left( \sqrt{\frac{2}{\sin{(\gamma)}}} \frac{(N-2)^{N - \frac{3}{2}}}{(N-1)^{\frac{N-1}{2}}} \frac{\ee^{N\left(-3\cos^2{(\gamma)} + \frac{1}{2} \right) + 1}}{\ee^{\left(3\cos^2{(\gamma)} - \frac{1}{2} \right)}}\right) \\
	& = \frac{2}{\sin{(\gamma)}} \left(\frac{(N-1)(N-2)^{2-3N^{-1}}}{N}\right)^{\frac{N}{2}} \frac{\ee^{2N\left(-3\cos^2{(\gamma)} + \frac{1}{2} \right) + 1}}{\ee^{-3\cos^2{(\gamma)} + \frac{1}{2}}}
\end{align}
and
\begin{equation}
	W_{(N,x,y)} = \frac{1}{\sqrt{2\pi}} \ee^{-2N\cos{(\gamma)}}
	\exp{\left\{ -\left(\sin{(2\gamma)} (x+y) +  \frac{\sin^2{(\gamma)}}{2N} (x^2 + y^2) \right) \right\}} 
\end{equation}
...
\begin{thm}
	...
	\begin{multline}
		K_n^{(2)}\left(\Delta_x, \Delta_y\right) = \frac{2 N^{\frac{1}{2}}}{\sin{(\delta)}\sqrt{2\pi}}
		\exp{\left\{-\frac{1}{2}\left(\sin{(2\gamma)} (x+y) +  \frac{\sin^2{(\gamma)}}{N} (x^2 + y^2) \right) - 3\left(\cos{(\gamma)} - \frac{1}{2}\right) \right\}} \\ \left(\frac{(N-1)(N-2)^{2-\frac{3}{N}}}{N}\right)^{\frac{N}{2}} \Sine(x, y) (1 + \Boh(N^{-1}))
	\end{multline}
\end{thm}
