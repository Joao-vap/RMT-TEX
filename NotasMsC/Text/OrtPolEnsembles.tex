...
We start illustrating the results above for the case $\beta=1$. Begin with the following characterization of the Jacobian as a Vandermonde determinant illustrated in \ref{Vandermonde}
\begin{equation}
...
\begin{equation}
	\pe^{(w)}(\lambda) = K_n^{(2)}(\lambda, \lambda) = \frac{\sqrt{\wf(x) \wf(y)}}{r_{n-1}^{(2)}} \left( q_{n-1}^{(2)}(\lambda)\partial_\lambda q_{n}^{(2)}(\lambda) - q_{n}^{(2)}(\lambda)\partial_\lambda q_{n-1}^{(2)}(\lambda)  \right)
\end{equation}
