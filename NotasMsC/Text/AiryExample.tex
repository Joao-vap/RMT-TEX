...
\begin{definition}
	(Airy function) Let $\langle$ be the union of the line segments $\{t\ee^{\ii \pi/3} : t \geq 0\}$ and $\{ t\ee^{-\ii\pi/3}: t \geq 0\}$, that is the union of the two infinite rays in the complex plane emanating from the origin that make	angles $\pi/3$ and $-\pi/3$ with the real axis. For $u \in \C$ the Airy function is defined by the
	complex contour integral
	\begin{equation}
		\Ai(u) = \frac{1}{2\pi\ii} \int_{\langle} \exp{\left(\frac{t^3}{3} - ut\right)} \dd t.
	\end{equation}
	\label{Def: Airy Function}
\end{definition}
Of course, by Cauchy Theorem, we know that the definition is independent of the contour as long as the contour goes from $\ee^{-2\pi\ii / 3}\infty$ to $\ee^{2\pi\ii / 3}\infty$. Now, we need to get things in order to apply Laplace method on the integral. We could start defining
...
%\begin{figure}[h] 
%	\centering
	%\subfigure{\includegraphics[width=0.49\textwidth]{./Assets/contour_plot_theta_0.10.png}}
	%\subfigure{\includegraphics[width=0.49\textwidth]{./Assets/contour_plot_theta_0.79.png}}
	%\subfigure{\includegraphics[width=0.49\textwidth]{./Assets/contour_plot_theta_1.57.png}}
	%\subfigure{\includegraphics[width=0.49\textwidth]{./Assets/contour_plot_theta_2.09.png}}
	%\subfigure{\includegraphics[width=0.49\textwidth]{./Assets/contour_plot_theta_2.36.png}}
	%\subfigure{\includegraphics[width=0.49\textwidth]{./Assets/contour_plot_theta_3.14.png}}
